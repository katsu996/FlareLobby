...
| `Glicko2Engine.calculate(input)` | `ratingA`、`ratingB`、A 側 `result`、省略可能な `deviationA` / `deviationB`（既定は初期 RD）                         | `Glicko2Calculation`。期待勝率、未丸め差分、整数差分、更新前後レート、更新後 RD・ボラティリティ、両側スコア           |
