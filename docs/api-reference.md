...
このページは、各パッケージの `src/index.ts` が公開する現行の API を、利用者
向けの引数・戻り値・イベント・エラーコードと一緒にまとめたものです。実装の型宣言
を正本とし、文書のコード例は `pnpm check:docs` で型検査します。v0.1.0 時点の
範囲と制限は歴史記録として [v0.1.0 Release Note](./releases/v0.1.0.md) を参照し、
現行仕様に書き換えません。現行版はパーティー、チーム単位のマッチング、
Glicko-2 を含みます。
...
| API                              | 引数                                                                                                                                                     | 戻り値                                                                                                                                 |
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| `glicko2(options?)`              | `initialRating?`（既定 1500）、`initialRatingDeviation?`（既定 350）、`tau?`（既定 0.5）、`volatility?`（既定 0.06）                                     | `Glicko2Engine`。`initialRating`、`initialRatingDeviation`、`tau`、`volatility`、`calculate()` を持つ凍結オブジェクト                  |
| `Glicko2Engine.calculate(input)` | `ratingA`、`ratingB`、A 側 `result`、省略可能な `deviationA` / `deviationB`（既定は初期 RD）と `volatilityA` / `volatilityB`（既定は初期ボラティリティ） | `Glicko2Calculation`。期待勝率、未丸め差分、整数差分、更新前後レート、試合前 RD・ボラティリティ、更新後 RD・ボラティリティ、両側スコア |

連戦では前回結果の `updatedRating*` / `updatedDeviation*` / `updatedVolatility*` を次の入力へ渡します。

`DEFAULT_GLICKO2_INITIAL_RATING`（1500）、`DEFAULT_GLICKO2_INITIAL_RATING_DEVIATION`（350）、`DEFAULT_GLICKO2_TAU`（0.5）、`DEFAULT_GLICKO2_VOLATILITY`（0.06）が既定値として公開されます。RD 入力は 0 以上の有限数、`tau`・`volatility`・初期 RD は正の有限数でなければなりません。ELO と異なり `deltaA + deltaB` は 0 になるとは限りません（ADR-0006）。
...
| `cors`             | 任意 `allowedOrigins`。正規の http/https Origin 文字列の配列。未設定時は同一オリジン向け挙動を維持                                                   |
| `observability`    | `logSampleRate?`、`analyticsSampleRate?`。既定は 1                                                                                                   |

必須 Binding は `FLARE_LOBBY_ROOMS`、`FLARE_LOBBY_MATCH_POOLS`、
`FLARE_LOBBY_PARTIES`、`FLARE_LOBBY_PARTY_MEMBERSHIPS`、
...
wildcard・文字列 `null` は `INVALID_CORS_CONFIGURATION` で拒否します。有効な
CORS プリフライトは認証と DO/D1 アクセスより先に `204` で応答し、許可 Origin の
通常応答（成功・`401`/`403`/`429`/`500` を含む）には `Access-Control-Allow-Origin` と
`Vary: Origin`、`Access-Control-Expose-Headers: Retry-After` を付けます。
CORS はサーバー側認可の代替ではありません。認可は `authorization` Hook で行います。
...
| `POST /v1/matchmaking/pools/{poolId}/tickets`                  | 必須（partyId 指定時はリーダー） | rating、region、inputMethod、検索属性、期限、`partyId?`、requestId                          | `201 + { ticket }`               |
...
| `POST /v1/matchmaking/pools/{poolId}/matches/{matchId}/result` | 認可 Hook                        | `resultId`、A 側 `result`。参加者はサーバー側で復元                                         | `{ match, applied }`             |
| `POST /v1/parties`                                             | 必須                             | `requestId`、`maxPartySize?`                                                                | `201 + { party }`                |
| `GET /v1/parties/{partyId}`                                    | メンバー                         | なし                                                                                        | `{ party }`                      |
| `POST /v1/parties/{partyId}/invites`                           | リーダー                         | `requestId`、`playerId`、`ttlMs?`                                                           | `{ invite }`                     |
| `POST /v1/parties/{partyId}/members`                           | 招待された主体                   | `requestId`、`token`                                                                        | `{ party }`                      |
| `POST /v1/parties/{partyId}/leave`                             | メンバー                         | `requestId?`                                                                                | `{ dissolved }`                  |
| `POST /v1/parties/{partyId}/transfer-leadership`               | リーダー                         | `requestId`、`playerId`                                                                     | `{ party }`                      |
| `POST /v1/parties/{partyId}/dissolve`                          | リーダー                         | `requestId?`                                                                                | `{ party }`                      |
| `GET /v1/parties/{partyId}/events` と `/events/ws`             | メンバー                         | `afterSequence?`、WebSocket                                                                 | Party Event 配列                 |
...
`searchAttributes`、`expiresAt`、`expiresAtMs` を加えた型です。パーティーチケットは
構成員ごとの `MatchmakingTicketMember` 配列と `party`（`partyId` と投入時
`revision`）を持ちます。`MatchmakingMatchIntent`
は `pending`、`initializing`、`matched`、`failed` と試行回数・結果・エラーコードを
持ちます。Durable Object の `fetch`、`webSocketMessage`、`alarm` は低レベルの
イベント入口です。
Gateway は Match Pool Durable Object へ `MatchPoolGatewayStub` 契約で転送し、
`createTicket` では `searchAttributes`、`expiresAt`、`ttlMs` も引き渡します。

### Party Durable Object

パーティーの正本は `PartyDurableObject`（1 パーティー = 1 DO）であり、所属の
不変条件は主体ごとの `PartyMembershipDurableObject` が検査します。既定定員は
`DEFAULT_PARTY_MAX_SIZE`、招待 TTL は `DEFAULT_PARTY_INVITE_TTL_MS`、無活動掃除は
`DEFAULT_PARTY_IDLE_TTL_MS` です。Gateway の入口は `handlePartyRequest`、
WebSocket 判定は `getPartyWebSocketRoute` と `upgradePartyEventsWebSocket` です。

| メソッド                              | 引数                                        | 戻り値                                    |
| ------------------------------------- | ------------------------------------------- | ----------------------------------------- |
| `createParty`                         | 主体、`requestId`、`maxPartySize?`          | `PartySnapshot`。作成者がリーダー         |
| `inviteMember`                        | リーダー、`requestId`、`playerId`、`ttlMs?` | `PartyInvite`。期限内未使用は再利用       |
| `acceptInvite`                        | 招待された主体、`requestId`、`token`        | `PartySnapshot`                           |
| `leaveParty`                          | 主体、`requestId?`                          | Snapshot/null。2 未満で自動解散           |
| `transferLeadership`                  | リーダー、`requestId`、`playerId`           | `PartySnapshot`                           |
| `dissolveParty`                       | リーダー、`requestId?`                      | 解散後 Snapshot。待機チケットをキャンセル |
| `getSnapshot` / `getEvents`           | Gateway Principal、`afterSequence?`         | Snapshot/Event 配列                       |
| `beginQueueTicket` / `endQueueTicket` | Match Pool からの凍結要求                   | `PartyQueueStartResult` / void            |

型は `PartyMember`、`PartyMemberRole`、`PartyInvite`、`PartyEvent`、
`PartyCreationOptions`、`PartyInviteOptions`、`PartyInviteAcceptanceOptions`、
`PartyOperationOptions`、`PartyLeadershipTransferOptions`、
`PartyQueueStartOptions`、`PartyQueueStartResult`、`PartyGatewayStub`、
`PartyEventsWebSocketRoute` です。リーダーの自発的退出では解散せず最古参メンバーへ
権限が移り、待機中の構成変更は拒否されます。
...
| `registerTeamMatchResult(database, pool, input, configuration?, maxRetries?)` / `recordTeamMatchResult` | D1、Pool、`resultId`、`matchId`、両チームの ID と構成員、A 側 result | `Promise<TeamMatchResultRegistration>` |
...
チーム対応の試合は `TeamMatchResultRegistrationInput` を受け付け、各構成員の更新は
相手チーム平均との ELO 差分です。戻り値の `TeamRatingMatchRecord` は `teamIds` と
`TeamRatingMatchParticipant` 配列を持ちます。
...
CORS の正規 Origin（scheme/host/任意 port）、観測 sampling（0〜1）を確認します。
