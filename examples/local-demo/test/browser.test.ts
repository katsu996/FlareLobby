...
vi.mock("@flarelobby/client", () => ({ createFlareLobbyClient: mocks.createClient }));
...
let customMoveButtons = ["rock", "paper", "scissors"].map((move) => Object.assign(new FakeElement(), { dataset: { customMove: move } }));
let rankedMoveButtons = ["rock", "paper", "scissors"].map((move) => Object.assign(new FakeElement(), { dataset: { rankedMove: move } }));
...
function roomSnapshot(kind: "custom" | "match", status = "waiting") {
  return {
    room: kind === "custom" ? { kind, id: "room-1", invitationCode: "invite-1" } : { kind, id: "room-2", matchId: "match-1" },
    state: { status }, revision: 1,
    participants: [
      { id: "demo:alice", kind: "player", ready: false, player: { id: "demo:alice" } },
      { id: "demo:bob", kind: "player", ready: true, player: { id: "demo:bob" } }
    ]
  };
}
...
function createRoom(kind: "custom" | "match") {
  ...
  let moveListener: ((message: { payload: { move: string }; sender?: { participantId: string } }) => void) | undefined;
  return {
    snapshot, participantId: "demo:alice", role: kind === "custom" ? "host" : "player", closed: false,
    subscribe: vi.fn(() => () => {}),
    onStatusChange: vi.fn((listener: (status: string) => void) => { listener("connected"); return () => {}; }),
    onMessage: vi.fn((_event: string, listener: typeof moveListener) => { moveListener = listener; return () => {}; }),
    setReady: vi.fn(async () => { snapshot.participants[0]!.ready = true; }),
    startMatch: vi.fn(async () => { snapshot.state.status = "active"; }),
    send: vi.fn(async () => { moveListener?.({ payload: { move: "rock" }, sender: { participantId: "demo:bob" } }); }),
    leave: vi.fn(async () => {})
  };
}
function installDom(): void {
  elements.clear();
  customMoveButtons = ["rock", "paper", "scissors"].map((move) => Object.assign(new FakeElement(), { dataset: { customMove: move } }));
  rankedMoveButtons = ["rock", "paper", "scissors"].map((move) => Object.assign(new FakeElement(), { dataset: { rankedMove: move } }));
  const values = new Map<string, string>();
  const globals = globalThis as unknown as Record<string, unknown>;
  globals["document"] = {
    getElementById: (id: string) => element(id),
    createElement: () => new FakeElement(),
    querySelectorAll: (selector: string) => selector === "[data-custom-move]" ? customMoveButtons : rankedMoveButtons
  };
  globals["localStorage"] = { getItem: (key: string) => values.get(key) ?? null, setItem: (key: string, value: string) => values.set(key, value) };
  globals["window"] = { location: { origin: "https://demo.test" }, setInterval: vi.fn(() => 1), clearInterval: vi.fn() };
}
...
function installClient(): void {
  ...
  ticket = {
    on: vi.fn((_event: string, listener: (value: unknown) => void) => {
      listener({ ticket: { status: "searching" }, searchWidth: 40, waitingTimeMs: 1000 });
      return () => {};
    }),
  ...
  client = {
    ...
    request: vi.fn(async () => ({
      matchId: "match-1", ready: true, yourMove: "rock", opponentMove: "scissors",
      result: { value: 1, outcome: "win", resultId: "result-1", applied: true }, rating: { value: 1250 }
    })),
    dispose: vi.fn()
  };
...
describe("ローカルデモの失敗・取消フロー", () => {
  it("カスタムルーム参加失敗を表示する", async () => {
    ...
    client.joinCustomRoom.mockRejectedValueOnce(new Error("参加に失敗しました"));
...
describe("ローカルデモのカスタムルーム参加", () => {
  it("招待コードで既存ルームへ参加して接続状態を表示する", async () => {
    ...
    expect(element("custom-move-actions").classList.contains("hidden")).toBe(true);
  });
});
