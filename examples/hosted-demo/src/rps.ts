/**
 * 公開デモのランク戦 API（手の受付・結果確定）です。
 *
 * local-demo の `src/rps.ts` と同じ方針です。成立済み Match のチケットから
 * 参加者を復元してサーバーが勝敗を決め、既存 `registerMatchResult` で冪等に
 * 反映します。クライアントの勝敗・参加者 ID 申告は採用しません。一般利用者
 * の `authorizeMatchResult` は false のため、この入口は手だけを受け付けます。
 */
import {
...
/** 公開サンプルのランク戦で利用するマッチメイキングプール ID です。 */
export const HOSTED_RANKED_POOL_ID = "ranked-jp";
...
/** 公開サンプルのランク戦 API を処理します。 */
export async function handleHostedRpsRequest<
...
export async function authenticateHostedRpsRequest<
  TApp extends AnyFlareLobbyApp = AnyFlareLobbyApp,
>(
  request: Request,
  configuration: FlareLobbyConfiguration<TApp>,
...
>(
  ...
  const poolConfiguration = configuration.matchmakingPools.find(
    (candidate) => candidate.id === HOSTED_RANKED_POOL_ID,
  );
