...
describe("@flarelobby/testing の決定論的テスト補助", () => {
  ...
  it("固定乱数と分布生成が同じ種で同じプレイヤー列を返す", () => {
    ...
    expect(first.every((item) => item.rating >= 1_400 && item.rating <= 1_700)).toBe(
      true
    );
  });
  ...
  it("待機時間に応じた検索幅拡大、成立、未成立チケットを再現する", () => {
    ...
    expect(result.tickets.find((ticket) => ticket.playerId === "a")?.status).toBe(
      "matched"
    );
    expect(result.tickets.find((ticket) => ticket.playerId === "c")?.status).toBe(
      "waiting"
    );
  });
