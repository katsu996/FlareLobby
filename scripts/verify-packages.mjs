...
const archiveDirectory = mkdtempSync(join(tmpdir(), "flarelobby-package-audit-"));
const errors = [];
const reports = [];
...
requireEqual(rootManifest.version, "0.1.0", "ルートの release version が不正です");
requireEqual(rootManifest.license, "MIT", "ルートの license が不正です");
requireEqual(changesetConfig.access, "public", "Changesets の公開範囲が不正です");
...
for (const packageDefinition of packages) {
  ...
  requireEqual(manifest.name, packageDefinition.name, `${manifestPath} の name が不正です`);
  requireEqual(manifest.version, "0.1.0", `${manifestPath} の version が不正です`);
  requireEqual(manifest.license, "MIT", `${manifestPath} の license が不正です`);
  requireEqual(manifest.type, "module", `${manifestPath} は ES Modules ではありません`);
  ...
  if (packageLicense !== rootLicense) {
    errors.push(`${packageDefinition.directory}/LICENSE がルートの MIT License と一致しません`);
  }
  if (!packageReadme.includes(packageDefinition.name) || !packageReadme.includes("pnpm add")) {
    errors.push(`${packageDefinition.directory}/README.md に package 名または導入例がありません`);
  }
  ...
  for (const entry of [exportDefinition?.types, exportDefinition?.import]) {
    if (typeof entry === "string" && !existsSync(resolve(root, packageDefinition.directory, entry))) {
      errors.push(`${manifestPath} の Entry Point が build 成果物にありません: ${entry}`);
    }
  }

  const filePatterns = new Set(Array.isArray(manifest.files) ? manifest.files : []);
  for (const pattern of ["dist", "!dist/.tsbuildinfo", "README.md", "LICENSE"]) {
    if (!filePatterns.has(pattern)) {
      errors.push(`${manifestPath} の files に必要な許可パターンがありません: ${pattern}`);
    }
  }
  ...
  for (const dependencyName of dependencyNames) {
    if (manifest.dependencies?.[dependencyName] !== "workspace:*") {
      errors.push(`${manifestPath} の内部依存が workspace protocol ではありません: ${dependencyName}`);
    }
  }
  ...
  for (const requiredPath of [
    ...
    if (!packedFiles.includes(requiredPath)) {
      errors.push(`${packageDefinition.name} の npm package に必要なファイルがありません: ${requiredPath}`);
    }
  }

  for (const packedPath of packedFiles) {
    const allowedRootFile = ["LICENSE", "README.md", "package.json"].includes(packedPath);
    const allowedDistFile = packedPath.startsWith("dist/");
    if (!allowedRootFile && !allowedDistFile) {
      errors.push(`${packageDefinition.name} の npm package に不要なファイルがあります: ${packedPath}`);
    }
    if (
      ...
      errors.push(`${packageDefinition.name} の npm package に内部・秘密ファイルがあります: ${packedPath}`);
    }
  }
  ...
  if (packResult.status !== 0) {
    errors.push(`${packageDefinition.name} の tarball 生成が失敗しました:\n${packResult.stdout}${packResult.stderr}`);
    continue;
  }
  ...
  try {
    ...
    errors.push(`${packageDefinition.name} の pack 結果を読めません: ${String(error)}`);
    continue;
  }
  if (typeof archivePath !== "string") {
    errors.push(`${packageDefinition.name} の pack 結果に tarball path がありません`);
    continue;
  }

  const packedManifestResult = spawnSync(tar, ["-xOf", archivePath, "package/package.json"], {
    cwd: root,
    encoding: "utf8",
  ...
  try {
    ...
    errors.push(`${packageDefinition.name} の tarball manifest が不正です: ${String(error)}`);
    continue;
  }

  if (JSON.stringify(packedManifest).includes("workspace:")) {
    errors.push(`${packageDefinition.name} の tarball manifest に workspace protocol が残っています`);
  }
...
if (errors.length > 0) {
  ...
  console.log("package metadata、Entry Point、型定義、依存関係、MIT License、公開内容の検証に成功しました。");
}
