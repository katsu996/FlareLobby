...
const outputDirectory = mkdtempSync(join(tmpdir(), "flarelobby-deploy-dry-run-"));
...
function run(arguments_) {
  ...
  if (result.status !== 0) {
    throw new Error(`${pnpm} ${arguments_.join(" ")} が失敗しました:\n${result.stdout}${result.stderr}`);
  }
...
try {
  ...
  if (!existsSync(bundlePath) || statSync(bundlePath).size === 0) {
    throw new Error("Wrangler dry-run の Worker bundle が生成されませんでした。");
  }

  const bundle = readFileSync(bundlePath, "utf8");
  if (bundle.includes("x-demo-player") && bundle.includes("FLARE_LOBBY_TOKEN_SECRET=demo")) {
  ...
  for (const requiredText of [
    ...
    if (!deployOutput.includes(requiredText)) {
      throw new Error(`Wrangler dry-run の出力に必要な確認項目がありません: ${requiredText}`);
    }
  }
